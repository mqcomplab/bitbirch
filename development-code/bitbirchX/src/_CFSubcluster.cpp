...
_CFSubcluster::_CFSubcluster(const xt::xarray<float>& linear_sum, const std::vector<int>& mol_indices) {
...
void _CFSubcluster::update(_CFSubcluster* subcluster) {
    ...
    this->mol_indices.reserve(this->mol_indices.size() + subcluster->mol_indices.size());
...
bool _CFSubcluster::merge_subcluster(_CFSubcluster* nominee_cluster, double threshold) {
    ...
    this->mol_indices.reserve(new_mol_indices.size() + nominee_cluster->mol_indices.size());
